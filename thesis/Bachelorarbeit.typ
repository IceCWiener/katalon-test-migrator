...
Ein häufig eingesetztes Verfahren ist der *End-to-End-Test* (E2E-Test), bei dem eine Anwendung aus Nutzerperspektive durch die Benutzeroberfläche getestet wird. Werkzeuge wie die Selenium WebDriver API ermöglichen solche Tests für webbasierte Anwendungen, indem sie einen Browser programmatisch steuern.
...
#figure(
  rect(width: 80%, height: 120pt, stroke: 1pt + gray, fill: rgb("#f7fafc"))[
    #set align(center + horizon)
    [Diagram Placeholder: Low-Code Scaling Inflection vs. Open-Source Freedom]
  ],
  caption: [Comparative overview of proprietary Low-Code platform scaling vs. Open-Source alternatives. (Source: Own work)],
) <fig-paradigm>

As shown in @fig-paradigm, the breaking curve represents technical licensing barriers.

= Conception of the Migration Pipeline
This section details your architectural design principles.

#figure(
  rect(width: 80%, height: 140pt, stroke: 1pt + gray, fill: rgb("#f7fafc"))[
    #set align(center + horizon)
    [Flowchart Diagram: Katalon Project Folder -> Parser -> Mapping Engine -> Selenium Outputs]
  ],
  caption: [System architecture of the automated Groovy-to-Python Regex transformation engine. (Source: Adapted from Smith et al., 2024, p. 12)],
) <fig-pipeline>

The technical pipeline architecture visualized above in @fig-pipeline details the precise structural transformation layers.

= Implementation of the Prototype
Detail the technical specifications of your written scripts and matching regex engine loops here.
